...
class MailboxClient:
    ...
    Credentials can be supplied explicitly via ``imap_credentials`` (an
    ``ImapCredentials`` dataclass from ``credentials.py``).  When *None* the
    client falls back to reading the legacy ``OUTLOOKPLUS_IMAP_*`` env vars so
    existing local-dev workflows keep working unchanged.

    Per-user credentials are not implemented here; ``user_id`` is kept for
    API shape.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._imap_credentials = imap_credentials

    def _resolve_credentials(self):
        """Return (host, port, username, password, folder) from explicit creds or env."""
        creds = self._imap_credentials
        if creds is not None:
            return creds.host, creds.port, creds.username, creds.password, creds.folder
    ...
    def list_new_messages(self, *, user_id: str, cursor: Optional[MailboxCursor]) -> list[RawMailboxMessage]:
        host, port, username, password, folder = self._resolve_credentials()
