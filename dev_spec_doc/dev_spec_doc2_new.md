...
> [!NOTE]
> Revision for harmonization (2026-03-04): updated persistence from JSON files to SQLite3 and added `GET /api/meeting/check` to expose meeting-related status to other backend modules.

<details>
<summary><strong>ORIGINAL vs REVISED (persistence)</strong></summary>
...
- SQLite3 database file stores email rows and classification rows.
- Attachment bytes remain on disk; SQLite stores attachment metadata + file path.
```
</details>
...
classDiagram
	...
	class MeetingService {
		+MeetingClassification getMeetingRelatedStatus(string userId, string emailId)
	}

	class MeetingApiController {
		+MeetingCheckDTO checkMeeting(string bearerToken, string messageId)
	}

	class MeetingCheckDTO {
		+string messageId
		+bool meetingRelated
	...
	MeetingApiController --> AuthTokenVerifier : verify token
	MeetingApiController --> MeetingService : lookup
	MeetingService --> EmailRepository : read classification
	MeetingApiController --> MeetingCheckDTO : return
```
...
- `MeetingService`: Provides reusable access to stored meeting classification results for other backend features (e.g., US3).
...
- `MeetingApiController`: Exposes a lightweight internal endpoint to fetch `meetingRelated` status for a given message id.
...
### `EmailRepository`
- Purpose: Persistence abstraction for emails and classifications (SQLite3-backed).
- Methods:
...
### `MeetingService`
- Purpose: Provides the meeting-related status for a message id using stored `MeetingClassification`.
- Methods:
	- `getMeetingRelatedStatus(userId: string, emailId: string): MeetingClassification`

### `MeetingApiController`
- Purpose: Implements the internal endpoint `GET /api/meeting/check`.
- Dependencies: `AuthTokenVerifier`, `MeetingService`.
- Methods:
	- `checkMeeting(bearerToken: string, messageId: string): MeetingCheckDTO`
...
## Persistence (SQLite3 + Attachment Files)
- Database: SQLite3
- Database File: `data/outlookplus.db`
- Structured Data Stored in SQLite:
	- Emails (normalized fields: subject/from/to/cc/sentAt/bodyText/hasAttachments)
	- Meeting classifications (US2)
	- Reply-need classifications + user feedback (US3)
- Attachment Storage (bytes on disk):
	- Path: `data/attachments/{userId}/{attachmentId}.bin`
	- SQLite stores attachment metadata (`mimeType`, `filename`) and `filePath`
- Concurrency Control:
	- SQLite in WAL mode
	- All writes performed inside transactions
	- File lock around attachment writes to prevent partial files
...
- Identifier naming: this spec may use `emailId` (path params) and `messageId` (query/body params). In the harmonized backend, they refer to the same value: the normalized email/message primary key.
...
- Purpose: Returns the stored meeting-related label for one email without returning email content.
- Auth: Required
- Query Parameters:
	- `messageId` (string, required; equals the stored `EmailMessage.id`)
- Response: `200 OK`
```json
{
	"messageId": "email_123",
...
- Error Responses:
	- `401 Unauthorized`
	- `404 Not Found`: message id not found for the authenticated user
...
@router.get("/meeting/check")
async def check_meeting(messageId: str, user_id: str = Depends(...)) -> dict: ...
```
...
```python
class MeetingService:
	async def get_meeting_related_status(self, user_id: str, message_id: str) -> "MeetingClassification":
		"""Returns stored MeetingClassification; raises NotFoundError if missing."""
		raise NotImplementedError
```
...
## Storage Interfaces (SQLite3 + Attachment Files)
...
class EmailRepository:
	...
	def get_email(self, user_id: str, message_id: str) -> "EmailMessage": ...
	...
	def get_classification(self, user_id: str, message_id: str) -> Optional["MeetingClassification"]: ...
```
...
> Revision Note (Harmonization): Persistence is SQLite3 (shared backend with US3). The JSON file store layout and persisted JSON schemas are preserved below as ORIGINAL historical text for traceability.

<details>
<summary><strong>ORIGINAL (prior version): JSON File Store persistence</strong></summary>
...
## Persistence (SQLite3) (Authoritative)
...
CREATE INDEX IF NOT EXISTS idx_emails_user_sent_at
...
CREATE TABLE IF NOT EXISTS email_attachments (
  ...
  FOREIGN KEY (user_id, message_id) REFERENCES emails(user_id, message_id)
);

CREATE INDEX IF NOT EXISTS idx_attachments_user_message
  ON email_attachments(user_id, message_id);
...
CREATE TABLE IF NOT EXISTS meeting_classifications (
  ...
  source TEXT NOT NULL CHECK (source IN ('gemini')),
...
### Attachment Bytes
- Attachment bytes may be stored on disk (e.g., under `DATA_DIR/uploads/`) and referenced by `email_attachments.storage_path`.
...
> Chat Log is historical; it may reference earlier, non-authoritative JSON file store details.
...
> Revision Note (Harmonization): The shared backend persists structured data in SQLite3; the prior JSON file store guidance is preserved below as ORIGINAL historical text.

<details>
<summary><strong>ORIGINAL (prior version): Storage Security (JSON File Store)</strong></summary>
...
- Database file location:
	- SQLite database file path is `SQLITE_DB_PATH` (default `data/outlookplus.db`).
	- The backend process user is the only account with read/write access to the DB file and its directory.
- Authorization enforcement:
	- All reads/writes MUST be scoped by `user_id` in SQL (`WHERE user_id = :user_id ...`).
	- Always use parameterized queries (no string interpolation).
- Attachment file safety:
	- Only `attachment_id` is used for filenames (server-generated UUID, regex `^[a-f0-9-]{36}$`).
	- `message_id` is treated as an opaque string and is never used in filesystem paths.
	- `storage_path` must be generated by the server and constrained under `DATA_DIR` (no user-provided paths).
- File integrity:
	- Attachment metadata includes `sha256` for `.bin` verification.
...
- Retention is enforced by a daily cleanup job that:
	- Deletes old rows from SQLite3 (scoped by `user_id`) using `sent_at` and `classified_at`.
	- Deletes attachment files referenced by rows removed from `email_attachments`.

Example cleanup logic (illustrative):
```sql
-- delete old meeting classifications
DELETE FROM meeting_classifications
WHERE classified_at < :cutoff_iso;

-- delete old emails
DELETE FROM emails
WHERE sent_at < :cutoff_iso;
```

Note: cleanup may be performed globally for all users (as above) or batched by `user_id`.
...
### RC-5: SQLite/attachment write concurrency causes corruption or lock failures
- Cause: Multiple ingestion runs contend on SQLite write locks, or concurrent attachment writes produce partial files.
- Impact: Ingestion fails, attachment reads fail, or feed endpoints return incomplete results.
- Mitigation:
	- Use SQLite transactions for multi-row writes and enable WAL mode.
	- Serialize ingestion runs per user to reduce write contention.
	- For attachments, use an atomic write pattern (write temp file then rename) plus a per-attachment lock.
- Owner: Backend
