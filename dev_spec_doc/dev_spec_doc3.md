...
**Dependencies:** RuleEngine, LlmClassifier, DecisionAggregator, OutputValidator, MeetingService (from US2)
...
**Logic:** 
- If rules are very confident (≥0.9), use them
- If rules return UNSURE or low confidence (<0.9), use LLM result
...
stateDiagram-v2
  ...
  Requested --> Processing: Backend starts
  Processing --> Shown: Classification complete
...
flowchart TD
  ...
  F --> G{Rule Confident?}
  G -->|Yes, confidence > 0.9| H[Use Rule Result]
  G -->|No, confidence ≤ 0.9| I[Call LLM API]
...
**Problem:** US3 depends on US2 being implemented and working correctly. If US2's API is unavailable or returns incorrect data, US3's classification quality degrades.
...
- Email content sent to OpenAI/Gemini for classification only
